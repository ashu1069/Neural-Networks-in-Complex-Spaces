...
def _save_anim(
    anim_obj: animation.FuncAnimation, name: str, fps: int = 24
) -> list[Path]:
...
def anim_dead_seed_mechanism():
    ...
    gs = fig.add_gridspec(
        2, 2, hspace=0.32, wspace=0.20, left=0.07, right=0.97, top=0.88, bottom=0.10
    )
    ...
    ax_g_crelu.set_title(
        "CReLU @ lr = 0.0236  (matched-shared-trial)", color=ACCENT_AMBER
    )
    ax_g_zrelu.set_title(
        "ZReLU @ lr = 0.0024  (matched-shared-trial)", color=ACCENT_LIME
    )
    ...
    ax_l_crelu.text(
        ...
        1.18,
        "the unstable regime",
        transform=ax_l_crelu.transAxes,
        ha="center",
        fontsize=10,
        color=ACCENT_AMBER,
        alpha=0.85,
    ...
    ax_l_zrelu.text(
        ...
        1.18,
        "the stable regime",
        transform=ax_l_zrelu.transAxes,
        ha="center",
        fontsize=10,
        color=ACCENT_LIME,
        alpha=0.85,
    ...
    def update(frame):
        ...
        def draw(traces, lines_g, lines_l):
            n_dead_so_far = 0
            for t, lg, ll in zip(traces, lines_g, lines_l, strict=False):
        ...
        if frame >= n_steps - 1:
            counter_text.set_text(
                f"end of replay  ▸  CReLU: {n_d_crelu}/9 dead seeds   ZReLU: {n_d_zrelu}/9 dead seeds"  # noqa: E501
            )
        else:
            counter_text.set_text("")
...
def anim_selection_rule_morph():
    ...
    bar_c = ax.bar(
        0, matched[0], width=0.5, color=ACCENT_BLUE, edgecolor=ACCENT_BLUE, linewidth=0
    )
    bar_r = ax.bar(
        1, matched[1], width=0.5, color=ACCENT_RED, edgecolor=ACCENT_RED, linewidth=0
    )
    txt_c = ax.text(
        0,
        matched[0] + 0.025,
    ...
    def update(frame):
        ...
        gap_color = (1 - t) * np.array([0.984, 0.749, 0.141]) + t * np.array(
            [0.639, 0.902, 0.208]
        )
...
def anim_conditional_advantage():
    ...
    ax.set_xticklabels(
        [c[0] for c in conditions], rotation=18, ha="right", fontsize=10, color=FG
    )
    ...
    bars_r = []
    crowns = []
    for i, (_label, _c, _r, _w) in enumerate(conditions):
        bc = ax.bar(
            i - 0.18,
            0,
            width=0.34,
        ...
        txt = ax.text(
            i, 0, "", ha="center", color=FG, fontsize=13, fontweight="bold", alpha=0.0
        )
        crowns.append(txt)
    ...
    def update(frame):
        ...
        if frame >= len(conditions) * (reveal_frames_per + hold_after_reveal):
            summary_text.set_text(
                f"complex wins {cumulative_c}/{len(conditions)}  —  ranking inverts on QAM, low-SNR, fixed rotation"  # noqa: E501
            )
...
def anim_activation_sweep():
    ...
    data = {
        act: _collect(RESULTS / "synthetic_rf_telemetry" / act)
    ...
    def update(frame):
        ...
        for j, (act, _lr, _regime) in enumerate(activations):
            n_dead = 0
            for t, lg, ll in zip(
                data[act], grad_lines[act], loss_lines[act], strict=False
            ):  # noqa: E501
                lg.set_data(t["step"][: s + 1], t["head"][: s + 1])
                ll.set_data(t["step"][: s + 1], t["loss"][: s + 1])
                will_die = t["dead"] and s >= 2
        ...
        if frame >= n_steps - 1:
            ...
            global_summary.set_text(
                f"end of replay  ▸  unstable activations: {unstable_dead}/27 dead seeds   "  # noqa: E501
...
def anim_activation_sweep_full(
    *,
    ...
    data = {
        act: _collect(RESULTS / "synthetic_rf_telemetry_full" / act)
        for act, *_ in activations
    }
    ...
    def update(frame):
        ...
        if frame >= early_frames + late_frames:
            ...
            global_summary.set_text(
                f"end of training  ▸  unstable activations: {unstable_dead}/27 dead seeds   "  # noqa: E501
                f"·   stable activations: {stable_dead}/18 dead seeds"
            )
            global_summary.set_alpha(0.95)
        else:
        ...
        for act, *_ in activations:
            ...
            all_artists.extend(loss_lines[act])
        return (
            all_artists + dead_counters + [step_clock, progress_label, global_summary]
        )
...
def anim_activation_sweep_twitter():
    ...
    intro_q = intro_ax.text(
        0.5,
        0.66,
        "“Why does my CVNN beat real baselines by 23 pp?”",
        ha="center",
        va="center",
        fontsize=26,
        color=FG,
    ...
    intro_setup = intro_ax.text(
        ...
        "Watching the first 30 training steps for 9 real-baseline seeds per activation.",  # noqa: E501
        ha="center",
        va="center",
        fontsize=14,
        color=MUTED,
    ...
    outro_ax.set_xticks([])
    outro_ax.set_yticks([])
    outro_ax.set_xlim(0, 1)
    outro_ax.set_ylim(0, 1)
    ...
    outro_lines = [
        (
            "3 activations  →  real baselines die at step 1   (8 / 27 dead seeds)",
            ACCENT_AMBER,
            0.62,
        ),
        (
            "2 activations  →  real baselines train fine       (0 / 18 dead seeds)",
            ACCENT_LIME,
            0.54,
        ),
    ]
    ...
    def update(frame):
        if frame < intro_frames:
            ...
            for act, *_ in activations:
                for lg, ll in zip(grad_lines[act], loss_lines[act], strict=False):
                    lg.set_alpha(0.0)
                    ll.set_alpha(0.0)
            ...
            for act, *_ in activations:
                for lg, ll in zip(grad_lines[act], loss_lines[act], strict=False):
                    lg.set_alpha(
                        max(0.0, lg.get_alpha() - t * 0.6) if lg.get_alpha() else 0.0
                    )
                    ll.set_alpha(
                        max(0.0, ll.get_alpha() - t * 0.6) if ll.get_alpha() else 0.0
                    )
...
def main() -> None:
    ...
    anim_activation_sweep_full(
        figsize=(16, 9),
        out_name="anim_activation_sweep_full_16x9",
        title_size=18,
        subtitle_size=12,
        panel_title_size=13,
    )
