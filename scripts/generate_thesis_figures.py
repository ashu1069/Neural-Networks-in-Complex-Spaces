...
MAG_CMAP = LinearSegmentedColormap.from_list("warmsand", ["#fdfdfd", MAG_C, "#7a4f24"])
...
def _add_awgn(sym: np.ndarray, snr_db: float, rng: np.random.Generator) -> np.ndarray:
...
def fig_iq_anatomy() -> None:
    ...
    gs = fig.add_gridspec(
        2, 4, width_ratios=[1.15, 0.85, 0.85, 1.05], hspace=0.35, wspace=0.25
    )
    ...
    rows = [("PSK-8", psk, PHASE_C, "phase"), ("QAM-16", qam, MAG_C, "magnitude")]

    for r, (name, sym, accent, _winner) in enumerate(rows):
        # ---- col 1: constellation -----------------------------------------
        ax = fig.add_subplot(gs[r, 0])
        ax.set_aspect("equal")
        ax.scatter(
            sym.real, sym.imag, s=10, color=accent, alpha=0.55, edgecolors="none"
        )
        ...
        if "PSK" in name:
            ...
            circ = patches.Circle(
                (0, 0), 1.0, fill=False, edgecolor=MUTED, linewidth=0.6, ls="--"
            )
            ax.add_patch(circ)
        else:
            side = 4
        ...
        ax.set_xticks([-1, 0, 1])
        ax.set_yticks([-1, 0, 1])
        ax.set_xlabel("I (Re z)")
        ax.set_ylabel("Q (Im z)")
        ax.set_title(f"{name}: where the symbols live", loc="left", color=INK, pad=4)
        ...
        if "PSK" in name:
            ...
            ax.text(
                ...
                fontsize=9,
                color=INK,
                bbox=dict(boxstyle="round,pad=0.35", fc="#fff7e8", ec=MAG_C, lw=0.8),
            )
        else:
            ax.set_title("magnitude-only sees:", loc="left", pad=4)
            ax.text(
        ...
        if "PSK" in name:
            ...
            ax.text(
                ...
                fontsize=9,
                color=INK,
                bbox=dict(boxstyle="round,pad=0.35", fc="#eaf2ec", ec=PHASE_C, lw=0.8),
            )
        else:
            ax.set_title("phase-only sees:", loc="left", pad=4)
            ax.text(
        ...
        for y, lbl, v, c in zip(bar_y, bar_labels, bar_vals, bar_colors, strict=False):
            w = 0.55 * v
            ...
            ax.text(
                0.0, y + 0.055, lbl, transform=ax.transAxes, fontsize=8, color=MUTED
            )
        ax.text(
            0.0,
            0.04,
            "chance = 0.33 (3-class)" if "PSK" in name else "chance = 0.33 (3-class)",
            transform=ax.transAxes,
            fontsize=8,
            color=MUTED,
            style="italic",
...
def fig_liouville_trilemma() -> None:
    ...
    for anchor, vertex, drop_text, kept, names, fc, ec in cells:
        ...
        ud = direction / nd
        # exit cell perimeter
        t = min((w / 2) / max(abs(ud[0]), 1e-6), (h / 2) / max(abs(ud[1]), 1e-6))
        ...
        ax.annotate(
            ...
            xy=end,
            xytext=start,
            arrowprops=dict(arrowstyle="->", color=ec, lw=1.4, shrinkA=0, shrinkB=0),
        )
...
def fig_activation_atlas() -> None:
    ...
    fig, axes = plt.subplots(6, 3, figsize=(11.5, 19.0), constrained_layout=True)
    ...
    for r, (name, fn) in enumerate(activations):
        ...
        ax.set_title("|σ(z)|", loc="left", fontsize=10.5, color=INK, pad=2)
        ...
        ax = axes[r, 1]
        im = ax.imshow(
            ang, cmap=PHASE_CMAP, extent=extent, origin="lower", vmin=-np.pi, vmax=np.pi
        )
        ax.set_title("arg σ(z)", loc="left", fontsize=10, color=INK, pad=2)
        _atlas_axes(ax)
        cb = plt.colorbar(im, ax=ax, fraction=0.045, pad=0.02, ticks=[-np.pi, 0, np.pi])
        ...
        ax.set_title(
            "|∂_z̄ σ|  (0 ⇔ holomorphic)", loc="left", fontsize=10, color=INK, pad=2
        )
        _atlas_axes(ax)
        plt.colorbar(im, ax=ax, fraction=0.045, pad=0.02)
