...
\title{Complex spaces as geometry and optimization:\\
disentangling architectural priors from training robustness\\
...
phase-bearing data, but the phrase ``complex helps'' hides two
different mechanisms. We study CVNNs at a scale where both mechanisms
can be measured directly and argue that complex spaces buy
\emph{geometry and optimization}, not free accuracy. On the geometry
side, bias-free \code{ComplexConv1d} is exactly phase-equivariant; a
magnitude readout turns equivariant features into phase-invariant
logits when the intervening stack preserves or learns the symmetry. On
the optimization side, matched-shared-trial hyperparameter selection
reveals a robustness asymmetry: real-valued baselines at matched
capacity can suffer a step-1 explosion-into-dead-region failure mode on
the classifier head that complex networks tolerate. Per-step gradient
telemetry across $5 \times 4 \times 3 = 60$ RadioML runs localizes the
explosion to \code{head.weight}: under three high-lr activation
regimes, real baselines have 50--80$\times$ larger step-1 head
gradients than the complex model and produce $3/9$ dead real-baseline
seeds; under two lower-lr regimes the gradients stay bounded and the
apparent CVNN-vs-real gap collapses to $\pm 3$ percentage points. The
same threshold behavior replicates on synthetic AWGN-only data with
the same matched-trial hyperparameters, indicating an hp-driven rather
than RadioML-specific mechanism. We therefore separate three claims:
complex geometry is useful when the task contains phase/sequence
structure, complex parameterization changes optimization tolerance,
and matched-shared-trial accuracy should be read as a robustness stress
test rather than as an independently tuned performance estimate. The
1-D scope, three-class RadioML 2018.01A subset, and modest hardware are
deliberate: they make a complete mechanism analysis reproducible from
released code, manifests, telemetry traces, and figure scripts.
\end{abstract}
...
activation rows. The companion repository currently has 123 passing
...
complex conv stack can spend less capacity learning the relevant
rotation algebra. The observed wins are consistent with that prior,
without requiring every activation in the stack to preserve exact
equivariance.
...
\paragraph{Independent-winner contrast.} Re-doing the same table with
independent per-family hyperparameter selection (each family's best of
16 trials, rather than shared trial index) reduces the CReLU gap from
0.7293 vs 0.4999 (best matched real) to 0.7293 vs 0.7047 (best
independently selected real). The reduction comes from independent selection
allowing real baselines to avoid the high-lr trial that triggers the
step-1 explosion. We therefore read matched-shared-trial as a
robustness stress test and independent selection as the better estimate
of best achievable performance under separate tuning.
