...
- `CReLU(z) = ReLU(Re z) + i·ReLU(Im z)` — split, not holomorphic.
- `zReLU(z) = z if arg(z) ∈ [0, π/2] else 0` — phase-gated.
- `modReLU(z) = ReLU(|z| + b) · z / |z|` — magnitude-gated, preserves phase.
- `complex_cardioid(z) = 0.5 · (1 + cos(arg z)) · z`.
- `Siglog(z) = z / (1 + |z|)` — bounded, smooth, not holomorphic.
- `complex_tanh(z) = tanh(z)` — meromorphic with poles; included as a cautionary
  baseline.

For each activation, produce a one-page **characterization report**:

- Domain plot (heatmap of `|f(z)|` and `arg f(z)` over a grid in `ℂ`).
- Where is it (non-)holomorphic? Cauchy–Riemann residual map.
- Singularities / blow-ups.
- Edge-case definition at `z = 0`, especially for `arg(z)` and `z / |z|`.
- Empirical gradient norm distribution at init for a fixed MLP.

**Exit criterion:** characterization notebook committed for each activation;
results summarized in a single comparison table with fixed metrics and seeds.
